...
pcsTokens = []      # processed token
for t in tokens:
    if t:         # In python empty string is false/0
        pcsTokens.append(t)
        
# print(pcsTokens,"\n")

# count=len(tokens) - tokens.count('')
# print(count)
# print(len(tokens))
...
# 5) min Len and max Len of function


minLen, maxLen = len(Sentence[0]) , len(Sentence[0])

for i in range(len(Sentence)):
    if minLen > len(Sentence[i]):
        minLen = len(Sentence[i])
